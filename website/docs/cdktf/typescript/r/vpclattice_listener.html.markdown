...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    new VpclatticeService(this, "test", {
      name: "example",
    });
    const example = new VpclatticeTargetGroup(this, "example", {
      config: {
        port: 80,
        protocol: "HTTP",
        vpcIdentifier: Token.asString(awsVpcTest.id),
      },
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    new VpclatticeListener(this, "example", {
      defaultAction: {
        forward: [
          {
            targetGroups: [
...
Import VPC Lattice Listener using the `listenerId` of the listener and the `id` of the VPC Lattice service combined with a `/` character. For example:

```
$ terraform import aws_vpclattice_listener.example svc-1a2b3c4d/listener-987654321
```

<!-- cache-key: cdktf-0.17.1 input-01cbe5c196214567de077692dd25e6fd604d492f729a02d37cec878427e8b386 -->